...
# this script is perfectly safe if run as nromal user, you may need to be root
# to emerge wgetpaste though 

 
PATH="${PATH}:/usr/sbin" #this is needed for utils like lspci
...
else "please emerge wgetpaste (run ias root 'emerge wgetpaste') and then run this script again"; exit 1;
fi

#checking if you have gentools
if [ -f "/usr/bin/equery" ] ; then
    echo "gentools found, continuing"
else 
    echo "please emerge gentools, you will benefit from the tools like equery or revdep-rebuild, others will ask you to emerge them anyways"
fi
